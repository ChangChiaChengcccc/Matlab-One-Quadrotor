...
multirotor.E = [0.8;1;1.1;0.9]; % set E
multirotor.dt = dt;
...
multirotor.a = zeros(3, length(multirotor.t));
multirotor.R = zeros(9, length(multirotor.t));
multirotor.W = zeros(3, length(multirotor.t));
multirotor.dW = zeros(3, length(multirotor.t));
...
multirotor.fault_force_moment = zeros(4, length(multirotor.t));
multirotor.fault_rotor_thrust = zeros(4, length(multirotor.t));
...
for i = 2:length(multirotor.t)
    ...
    % control with efficiency
    controlE = cal_controlE(control,multirotor);
    ...
    X0 = [vec_enu_to_ned(multirotor.x(:, i-1));
        ...
        multirotor.W(:, i-1)];
    [T, X_new] = ode45(@(t, x) multirotor.dynamics(t, x, controlE), [0, dt], X0, controlE);
    ...
    [multirotor.a(:, i), multirotor.dW(:, i)] = dvdW(multirotor,i,controlE);
    ...
    % save true/fault rotor thrust  
    multirotor.force_moment(:, i) = control(1:4);
    multirotor.rotor_thrust(:, i) = multirotor.allocation_matrix_inv*control(1:4);
    multirotor.fault_force_moment(:, i) = controlE(1:4);
    multirotor.fault_rotor_thrust(:, i) = multirotor.allocation_matrix_inv*controlE(1:4);
end

ukf_state_estimation(multirotor);
